...
sadd({Bucket, Key}, Item) ->
    {ok, ReqId} = udon_op_fsm:op(?N, ?W, {sadd, {Bucket, Key}, Item}, {Bucket, Key}, ?TIMEOUT, true),
...
srem({Bucket, Key}, Item) ->
    {ok, ReqId} = udon_op_fsm:op(?N, ?W, {srem, {Bucket, Key}, Item}, {Bucket, Key}, ?TIMEOUT, true),
    case wait_for_reqid(ReqId, ?TIMEOUT) of
        {ok, [ok, ok]} ->
            jiffy:encode({[{<<"status">>, 0}]});
        Error ->
...
%% @doc Delete a set
del(Bucket, Key) ->
    {ok, ReqId} = udon_op_fsm:op(?N, ?W, {del, Bucket, Key}, {Bucket, Key}, ?TIMEOUT, true),
    case wait_for_reqid(ReqId, ?TIMEOUT) of
        {ok, [ok, ok]} ->
            jiffy:encode({[{<<"status">>, 0}]});
        Error ->
...
smembers(Bucket, Key) ->
    {ok, ReqId} = udon_op_fsm:op(?N, ?R, {smembers, Bucket, Key}, {Bucket, Key}, ?TIMEOUT, true),
    case wait_for_reqid(ReqId, ?TIMEOUT) of
...
smembers2(Bucket, Key) ->
    {ok, ReqId} = udon_op_fsm:op(?N, ?R, {redis_address}, {Bucket, Key}, ?TIMEOUT, true),
    case wait_for_reqid(ReqId, ?TIMEOUT) of
        {ok, RedisAddresses} ->
            WorkerPid = self(),
...
transaction({Bucket, Key}, CommandList) ->
    {ok, ReqId} = udon_op_fsm:op(?N, ?W, {transaction, {Bucket, Key}, CommandList}, {Bucket, Key}, ?TIMEOUT, true),
